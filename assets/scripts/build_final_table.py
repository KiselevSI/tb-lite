...
DISPLAY_SUBHEADER_MAP: Dict[str, str] = {
...
UNNAMED_HEADER_RE = re.compile(r"^Unnamed:\s*\d+_level_\d+$", re.IGNORECASE)
...
def _normalize_header_part(value: object) -> str:
    if value is None:
        return ""
    if pd.isna(value):
        return ""
    ...
    if not text or UNNAMED_HEADER_RE.match(text):
        return ""
    return text


def _display_subheader(col: str, sub_map: Dict[str, str]) -> str:
    if col not in sub_map:
        return str(col)

    sub = _normalize_header_part(sub_map[col])
    sub_norm = re.sub(r"\s+", "_", sub.lower())
    return DISPLAY_SUBHEADER_MAP.get(sub_norm, sub or str(col))
...
def build_header_rows(
    ...
    for col in columns:
        col_name = str(col)
        top_row.append(_normalize_header_part(top_map.get(col_name, "")))
        sub_row.append(_display_subheader(col_name, sub_map))

    return top_row, sub_row


def _normalize_codon_value(value: object) -> str:
    if value is None or pd.isna(value):
        return ""
    return str(value)


def _subheader_norm(value: object) -> str:
    return re.sub(r"\s+", "_", _normalize_header_part(value).lower())
...
def format_alt_codon_columns(
    ...
    alt_cols: Dict[str, int] = {}

    for idx, col in enumerate(write_df.columns):
        col_name = str(col)
        top = _normalize_header_part(top_map.get(col_name, ""))
        sub_norm = _subheader_norm(sub_map.get(col_name, ""))

        if not top:
            continue
        if sub_norm == "реф-й_кодон":
            ref_cols[top] = idx
        elif sub_norm == "кодон_с_заменой":
            alt_cols[top] = idx

    if not ref_cols or not alt_cols:
        return

    default_fmt = workbook.add_format({})
    changed_fmt = workbook.add_format({"font_color": "#FF0000"})

    for drug, alt_idx in alt_cols.items():
        ref_idx = ref_cols.get(drug)
        if ref_idx is None:
            continue

        for row_offset, row in enumerate(write_df.itertuples(index=False, name=None)):
            ref_value = _normalize_codon_value(row[ref_idx])
            alt_value = _normalize_codon_value(row[alt_idx])
            segments = build_alt_codon_segments(alt_value, ref_value, default_fmt, changed_fmt)
            if segments is None:
                continue

            excel_row = start_row + row_offset
            if len(segments) == 2:
                ws.write_string(excel_row, alt_idx, alt_value, changed_fmt)
            else:
                ws.write_rich_string(excel_row, alt_idx, *segments)
...
def dr_to_flat_with_maps(path: Union[str, Path]) -> tuple[pd.DataFrame, Dict[str, str], Dict[str, str]]:
    ...
    sample_cols = [
        ...
        and any(_normalize_header_part(c[i]).lower() == "sample" for i in range(len(c)))
    ]
    if not sample_cols:
    ...
    for col in raw.columns:
        ...
        top_s = _normalize_header_part(top)
        sub_s = _normalize_header_part(sub)
...
def main():
    ...
    if a.out.lower().endswith(".xlsx"):
        with pd.ExcelWriter(a.out, engine="xlsxwriter") as writer:
            if has_dr:
                data_start_row = 2
                write_df.to_excel(writer, sheet_name="Data", startrow=data_start_row, index=False, header=False)
                ws = writer.sheets["Data"]
                top_row, sub_row = build_header_rows(list(write_df.columns), top_map, sub_map)

                top_fmt = writer.book.add_format({"bold": True, "align": "center", "valign": "vcenter", "border": 1})
                sub_fmt = writer.book.add_format({"bold": True, "align": "center", "valign": "vcenter", "border": 1})

                for idx in range(len(top_row)):
                    ws.write_blank(0, idx, None, top_fmt)
                    ws.write(1, idx, sub_row[idx], sub_fmt)

                col = 0
                while col < len(top_row):
                    top = top_row[col]
                    if not top:
                        col += 1
                        continue
                    ...
                    while end + 1 < len(top_row) and top_row[end + 1] == top:
                        end += 1

                    if end > col:
                        ws.merge_range(0, col, 0, end, top, top_fmt)
                    else:
                        ws.write(0, col, top, top_fmt)
                    col = end + 1

                format_alt_codon_columns(
                    ws=ws,
