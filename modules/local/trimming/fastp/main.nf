process FASTP {
    ...
    if (files.size() == 2) {
        ...
        def read2 = files[1]
        """
        fastp --thread ${task.cpus} -i $read1 -I $read2 -o ${sample_name}_trimmed_1.fastq.gz -O ${sample_name}_trimmed_2.fastq.gz --detect_adapter_for_pe --trim_poly_g
        """
    } else if (files.size() == 1) {
        def read1 = files[0]
        """
        fastp --thread ${task.cpus} -i $read1 -o ${sample_name}_trimmed_1.fastq.gz --trim_poly_g
        """
    }
}